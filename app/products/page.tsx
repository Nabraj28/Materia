...
import { X, LayoutGrid, List, ChevronLeft, ChevronRight, Filter } from "lucide-react";
...
function ProductsContent() {
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(["Flooring Systems"]);
  ...
  const filteredProducts = useMemo(() => {
    return PRODUCTS_CATALOG.filter((product) => {
      ...
      // Thickness range
      const min = parseFloat(minThickness);
      const max = parseFloat(maxThickness);
      if (!isNaN(min) && product.thickness < min) return false;
      if (!isNaN(max) && product.thickness > max) return false;
      ...
      if (q) {
        ...
        if (!matchesName && !matchesCat && !matchesApp && !matchesTags) {
          return false;
        }
      }
  ...
  }, [selectedCategories, selectedApplications, minThickness, maxThickness, searchQuery]);
  ...
  const activeChips = useMemo(() => {
    ...
    if (minThickness || maxThickness) {
      chips.push({
        type: "Thickness",
        value: `${minThickness || "0"} - ${maxThickness || "∞"} mm`,
        onRemove: () => {
          setMinThickness("");
          setMaxThickness("");
        },
  ...
  return (
    <div className="flex-grow flex flex-col pt-16">
      ...
      >
        <div className="absolute inset-0 bg-[#f8f9fa]/70 backdrop-blur-[3px]" />
      ...
      {/* Main Content Layout */}
      <main className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-12 py-8 grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:hidden flex justify-between items-center bg-white p-3 border border-gray-200 rounded">
          ...
          >
            <Filter className="w-4 h-4 text-[#9c4000]" />
          ...
          {activeChips.length > 0 && (
            <button
              ...
              className="text-xs font-mono text-[#9c4000] hover:underline"
            >
              Clear All
            </button>
          )}
        ...
        >
          <div className="bg-white p-4 border border-gray-200 rounded space-y-5 shadow-xs">
            <div className="flex justify-between items-center border-b border-gray-200 pb-2">
              ...
              {activeChips.length > 0 && (
                <button
                  type="button"
                  onClick={clearAllFilters}
                  className="font-mono text-xs text-[#9c4000] hover:underline"
                >
                  Reset
                </button>
              )}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="h-4 w-4 text-[#9c4000] accent-[#9c4000] border-gray-300 rounded focus:ring-[#9c4000] cursor-pointer"
                        />
                        <span
                          className={`transition-colors ${
                            isChecked
                              ? "text-gray-900 font-medium"
                              : "text-gray-600 group-hover:text-[#9c4000]"
                          }`}
                        >
                          {cat}
                        </span>
            ...
            <div className="space-y-2 border-t border-gray-200 pt-4">
              <h3 className="text-sm font-semibold text-gray-900">Thickness (mm)</h3>
              <div className="flex items-center gap-2 pt-1">
                <input
                  type="number"
                  value={minThickness}
                  onChange={(e) => {
                    setMinThickness(e.target.value);
                    setCurrentPage(1);
                  }}
                  placeholder="Min"
                  className="w-full bg-white border border-gray-300 focus:border-[#9c4000] outline-none font-mono text-xs p-2 rounded focus:ring-1 focus:ring-[#9c4000]"
                />
                ...
                <input
                  ...
                  value={maxThickness}
                  onChange={(e) => {
                    setMaxThickness(e.target.value);
                    setCurrentPage(1);
                  }}
                  placeholder="Max"
                  className="w-full bg-white border border-gray-300 focus:border-[#9c4000] outline-none font-mono text-xs p-2 rounded focus:ring-1 focus:ring-[#9c4000]"
          ...
          {activeChips.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={clearAllFilters}
                className="font-mono text-xs text-[#9c4000] hover:underline px-2 py-1 cursor-pointer font-medium"
              >
                Clear All
              </button>
            </div>
          ...
          {paginatedProducts.length > 0 ? (
            ...
            <div className="bg-white border border-dashed border-gray-300 p-12 text-center rounded">
              ...
              <button
                type="button"
                onClick={clearAllFilters}
                className="px-4 py-2 bg-[#9c4000] text-white text-xs font-semibold rounded hover:bg-[#803400] transition-colors"
              >
                Clear Filters
              </button>
            </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center pt-6 gap-2">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:bg-gray-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
              >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`w-8 h-8 flex items-center justify-center rounded text-xs font-medium transition-colors cursor-pointer ${
                    currentPage === page
                      ? "bg-[#9c4000] text-white font-bold"
